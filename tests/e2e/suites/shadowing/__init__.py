...
match wins. The loser is otherwise invisible: a user who drops an override into
``~/.amplifier/skills`` has no way to see that a same-named file earlier in the search
order is the one actually running.

``GET /v1/skills`` returns, per skill, the winning ``source`` plus a ``shadowed`` list
naming every same-named file that lost. This suite proves the LAUNCHER carries that
